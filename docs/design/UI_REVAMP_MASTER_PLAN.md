...
## 🔗 Execution Bridge (May 2026 Audit Alignment)

To avoid design drift and trust regressions while revamping UI, execution is now anchored to the audited 4-screen remediation sequence:
...
- `docs/reports/audits/SPRINT_EXECUTION_PLAN_4_SCREENS_2026-05-24.md`

### Non-negotiable implementation rule

Visual upgrades must not ship ahead of contract-level correctness for:
- metric truth consistency,
- state authority clarity,
- filter/route contract fidelity,
- stale/fresh data signaling.

### Delivery order

1. **Stability + contract quick wins** (History ingress, stale reset, daily-goal truth source, taxonomy centralization)
2. **State and semantic consistency** (Result authority, reanalyze semantics, freshness indicators)
3. **Architecture consistency paydown** (sync policy, route policy, cloud/local history parity)

### Practical guidance

- If a planned UI enhancement conflicts with an open P1/P2 trust issue from the audit backlog, trust issue resolution takes precedence.
- Every remediation PR should include updated screenshots and a short “verified vs inferred” note.
