...
| 2026-05-22 | **10 tier-2 city plugins: AMC, SMC, JMC, LMC, NMC, IMC, BMC Bhopal, CCMC, Cochin, MCC** | `lib/services/city_policy_data.dart` + `local_guidelines_plugin.dart` + `local_policy_rule_packs.dart` | Added 10 tier-2 city instances with SWM 2016 national standard data + city-specific findings (helplines: LMC 6389200005, NMC SWM helpline, MCC 14420; notable: Indore #1 cleanest, Chandigarh pink bins, Kochi Brahmapuram crisis). All at `draft` stage. 44 total tests now cover all 17 cities. |
| 2026-05-22 | **SocietyPolicyService (Firestore CRUD)** | `lib/services/society_policy_service.dart` | Full Firestore service for society policies: get/set/update/delete, real-time stream, GPS proximity search (Haversine), verification. Collection `society_policies` added to `FirestoreCollections`. Policy engine accepts optional `societyId`. |
| 2026-05-22 | **PolicyProvenanceCard widget** | `lib/widgets/result_screen/policy_provenance_card.dart` | Shows authority, version, helpline, compliance status, last verified, rule pack ID. "Low confidence" badge when confidence-gated. "Why this matters" info dialog explaining deterministic policy. |
| 2026-05-22 | **RegionSelectionSection (settings)** | `lib/widgets/settings/region_selection_section.dart` | Bottom-sheet city picker in Settings screen with all 17 cities. Shows authority name and region. Wired into `enhanced_settings_screen.dart` between Privacy and Navigation sections. Exported via `settings_widgets.dart` barrel. |
| 2026-05-22 | **Policy source freshness GitHub Action** | `.github/workflows/policy-source-check.yml` | Weekly cron (Monday 9AM UTC) + manual trigger. HTTP HEADs each city's source URL, flags stale/past-review-date sources, files a GitHub issue with action links. Covers 7 metro cities initial targets. |
