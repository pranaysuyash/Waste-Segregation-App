...
| 9 | Release evidence / CI | ✅ **DONE** | `.github/workflows/ci.yml:81-104` — `functions:` job builds (`npm run build`) + tests (`npm test`); combined status job `needs: [analyze, firebase_rules, functions, test, golden, storybook, eval]` (`:190`); analyzer debt gate + `git diff --exit-code pubspec.yaml pubspec.lock` | Committed `fc6b7a49`. Matches Task 01 release-proof scope |
...
| ⚠️ *(uncommitted)* | **Task 03 round-3 corrective** — `firestore.rules` (clientCanCreate + diff-based updates), `firestore-rules-test/package.json` + `security-authz-test.spec.js` (SEC-01B/01C wired into gate), `lib/services/cloud_storage_service.dart` (strip server-owned keys), `test/services/firestore_schema_drift_test.dart` (anti-drift invariant) — **re-pin `firestore.rules` line anchors in this scorecard after this commit lands** |
...
dedicated test), 06 society policy authority, 08 sellable feature contracts, and the
storage-migration/back-compat half of 09. **Task 03 round-3 corrective (findings #1 and #2)
is complete and verified on disk — 136 emulator tests passing — but not yet committed.**
...
**Sequencing recommendation:** commit the uncommitted Task 03 corrective **and the C-09/C-10
webhook corrective** (finding #4) together, then continue — **release proof ✅, authorisation
(#1/#2 ✅, #3 residual), billing (#4 ✅ on disk → commit; #5/#6), image lifecycle (#8)**.
