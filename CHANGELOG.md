...
- **Code Quality Issues**: Fixed all linter warnings across multiple files
  - Replaced deprecated opacity calls with `withOpacity()` in quiz, image capture, auth, and main screens
...
- **🚫 Deprecated APIs**: Replaced deprecated opacity calls with `withOpacity()` for Flutter compatibility
