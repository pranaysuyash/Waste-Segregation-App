...
void main() {
  test('1. Eval schema validates good cases', () {
    final line = File('test/fixtures/ai_eval/golden_cases.jsonl')
        .readAsLinesSync()
        .first;
  ...
  test('5. Acceptable alternative can pass non-strictly', () {
    final c = EvalCase.fromJson(<String, dynamic>{
      ...
      'language': 'en',
      'expected': <String, dynamic>{'category': 'Wet Waste'},
      'acceptableAlternatives': <Map<String, dynamic>>[
        <String, dynamic>{'category': 'Reject Waste'}
      ],
      'mustNot': <String>[],
      'safetyCritical': false,
      'localRuleCritical': false,
  ...
  test('6. Offline eval mode requires no API keys', () async {
    final summary = await EvalRunner(fixtureRoot: 'test/fixtures/ai_eval')
        .run(mode: 'offline');
  ...
  test('7. Training candidate is not created when consent is false', () {
    expect(
      TrainingCandidatePolicy.shouldCreateCandidate(
          const TrainingConsentSnapshot(
              enabled: false, policyVersion: 'training-data-v1')),
      isFalse,
    );
  });
  ...
  test('8. Training candidate can be created when consent is true', () {
    expect(
      TrainingCandidatePolicy.shouldCreateCandidate(
          const TrainingConsentSnapshot(
              enabled: true, policyVersion: 'training-data-v1')),
      isTrue,
    );
  });

  test('9. Revoked/deleted/rejected candidates are excluded from export',
      () async {
    final lines = File(
            'test/fixtures/ai_eval/recorded_outputs/training_candidates_sample.jsonl')
        .readAsLinesSync();
    final raw =
        lines.map((l) => jsonDecode(l) as Map<String, dynamic>).toList();
    final outDir =
        Directory.systemTemp.createTempSync('dataset-export-test').path;
    final summary = await DatasetExporter().export(
        rawCandidates: raw, datasetVersion: 'waste-v0.1', outputDir: outDir);
  ...
  test('11. Dataset export produces deterministic manifest order', () async {
    final raw = <Map<String, dynamic>>[
      <String, dynamic>{
        ...
        'review': <String, dynamic>{'status': 'training_eligible'},
        'reviewerVerified': <String, dynamic>{
          'reviewedAt': '2026-05-23T00:00:00Z',
          'groundTruth': <String, dynamic>{'category': 'Dry Waste'}
        },
        'image': <String, dynamic>{'redactionStatus': 'pii_passed'}
      },
      <String, dynamic>{
        'candidateId': 'a',
        'consent': <String, dynamic>{
    ...
    final outDir =
        Directory.systemTemp.createTempSync('dataset-order-test').path;
    await DatasetExporter().export(
        rawCandidates: raw, datasetVersion: 'waste-v0.1', outputDir: outDir);
  ...
  test('12. Router comparison handles fake backend/local/provider outputs', () {
    final result = RouterMetrics.compare(
        provider: 'backend',
        outcomes: <Map<String, dynamic>>[
          <String, dynamic>{
            'strictPass': true,
            'acceptableAlternativePass': false,
            'safetyCriticalFailure': false,
            'mustNotViolation': false,
            'localRuleFailure': false,
            'multiItemFailure': false,
            'overconfidentWrong': false,
            'underconfidentCorrect': false,
            'latencyMs': 1000,
          ...
          <String, dynamic>{
            ...
            'localRuleFailure': true,
            'multiItemFailure': true,
            'overconfidentWrong': true,
            'underconfidentCorrect': false,
            'latencyMs': 1200,
