...
void main() {
  group('LocalPolicyRulePackRegistry', () {
    ...
    test('BBMP rule set uses deployed technical status', () {
      final pack = registry.getPackForPlugin('bbmp_bangalore');
      expect(pack.rules, isNotEmpty);
      expect(pack.governanceStage, equals('deployed'));
      expect(pack.sourceStatus, equals('verified'));
      expect(pack.authorityStatus, equals('reviewed_by_expert'));
    });
    ...
    test('every city pack has safety override rules', () {
      ...
      for (final id in allIds) {
        ...
        expect(
          ...
          hasLength(2),
          reason:
              '$id should have exactly 2 safety override rules (hazardous + medical)',
        );
      }
    });
