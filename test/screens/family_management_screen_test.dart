...
family_models.Family createMockFamily({
  ...
  return family_models.Family(
    ...
    members:
        members ?? [createMockFamilyMember(role: family_models.UserRole.admin)],
...
invitation_models.FamilyInvitation createMockInvitation({
  ...
  user_profile_models.UserRole roleToAssign = user_profile_models
      .UserRole.member, // Role from user_profile for invitation
...
void main() {
  ...
  late StreamController<List<user_profile_models.UserProfile>>
      membersStreamController;
  late StreamController<List<invitation_models.FamilyInvitation>>
      invitationsStreamController;

  setUp(() {
    mockFamilyService = MockFirebaseFamilyService();
    mockStorageService = MockStorageService();

    familyStreamController =
        StreamController<family_models.Family?>.broadcast();
    membersStreamController =
        StreamController<List<user_profile_models.UserProfile>>.broadcast();
    invitationsStreamController =
        StreamController<List<invitation_models.FamilyInvitation>>.broadcast();

    when(mockStorageService.getCurrentUserProfile()).thenAnswer((_) async =>
        createMockUserProfile(
            id: 'currentUserAdmin')); // Removed role from UserProfile mock creation
  ...
  Widget createTestableWidget(
      family_models.Family initialFamily, Widget child) {
    final testTheme = ThemeData.light().copyWith(
      primaryColor:
          AppTheme.primaryColor, // Assuming this is a static color on AppTheme
      colorScheme: ColorScheme.fromSeed(
          seedColor: AppTheme
              .primaryColor), // Assuming this is a static color on AppTheme
  ...
  final mockInitialFamily =
      createMockFamily(id: 'manageFam1', name: 'Manageable Family');

  group('FamilyManagementScreen Tests', () {
    testWidgets('Displays loading indicators for streams initially',
        (WidgetTester tester) async {
      when(mockStorageService.getCurrentUserProfile())
          .thenAnswer((_) async => createMockUserProfile(id: 'adminUser'));
      // Don't add data to controllers yet

      await tester.pumpWidget(createTestableWidget(mockInitialFamily,
          FamilyManagementScreen(family: mockInitialFamily)));

      // Main family stream loading
      expect(find.byType(CircularProgressIndicator),
          findsOneWidget); // For the main family stream
      await tester
          .pump(); // Let one frame pass for streams to establish connection state
      ...
      expect(find.byType(CircularProgressIndicator),
          findsOneWidget); // For members stream

      await tester.tap(find.text('Invitations'));
      await tester.pumpAndSettle();
      expect(find.byType(CircularProgressIndicator),
          findsOneWidget); // For invitations stream
    });

    testWidgets('Displays family name in AppBar from stream',
        (WidgetTester tester) async {
      ...
      await tester.pumpWidget(
          createTestableWidget(family, FamilyManagementScreen(family: family)));
      familyStreamController.add(family); // Emit family data
      membersStreamController
          .add([]); // Emit empty members to resolve members stream
      invitationsStreamController
          .add([]); // Emit empty invitations to resolve invitations stream
      await tester.pumpAndSettle();

      expect(find.text('Manage Streamed Family Name'), findsOneWidget);
    });

    testWidgets('Displays members in Members tab from stream',
        (WidgetTester tester) async {
    ...
    testWidgets('Displays invitations in Invitations tab from stream',
        (WidgetTester tester) async {
      final family = createMockFamily();
      final invitations = [
    ...
    group('Settings Tab Actions', () {
      final familyForSettings =
          createMockFamily(id: 'settingsFam', name: 'Settings Test Fam');

      setUp(() {
        // Ensure current user is admin for settings modification
        when(mockStorageService.getCurrentUserProfile()).thenAnswer((_) async =>
            createMockUserProfile(
                id: 'adminUser', familyId: familyForSettings.id));

        // Ensure updateFamily is stubbed
        when(mockFamilyService.updateFamily(any)).thenAnswer((_) async {});
      });

      testWidgets('Edit Family Name shows dialog and calls updateFamily',
          (WidgetTester tester) async {
        await tester.pumpWidget(createTestableWidget(familyForSettings,
            FamilyManagementScreen(family: familyForSettings)));
        ...
        expect(find.widgetWithText(TextField, familyForSettings.name),
            findsOneWidget);
        ...
        final updatedFamily =
            familyForSettings.copyWith(name: 'New Family Name');
        verify(mockFamilyService.updateFamily(argThat(
                predicate<family_models.Family>((f) =>
                    f.name == updatedFamily.name && f.id == updatedFamily.id))))
            .called(1);
        expect(find.text('Family name updated!'), findsOneWidget); // SnackBar
      });

      testWidgets('Copy Family ID calls Clipboard.setData and shows SnackBar',
          (WidgetTester tester) async {
        await tester.pumpWidget(createTestableWidget(familyForSettings,
            FamilyManagementScreen(family: familyForSettings)));
        familyStreamController.add(familyForSettings);
      ...
      testWidgets(
          'Toggle "Public Family" calls updateFamily and shows SnackBar',
          (WidgetTester tester) async {
        final initialSettings = family_models.FamilySettings.defaultSettings()
            .copyWith(isPublic: false);
        final familyWithSettings = createMockFamily(settings: initialSettings);

        await tester.pumpWidget(createTestableWidget(familyWithSettings,
            FamilyManagementScreen(family: familyWithSettings)));
        ...
        final publicSwitch =
            find.widgetWithText(SwitchListTile, 'Public Family');
        ...
        verify(mockFamilyService.updateFamily(argThat(
                predicate<family_models.Family>(
                    (f) => f.settings.isPublic == expectedSettings.isPublic))))
            .called(1);
        expect(
            find.text('Public family setting updated to true'), findsOneWidget);
      });

      testWidgets(
          'Toggle "Share Classifications" calls updateFamily and shows SnackBar',
          (WidgetTester tester) async {
        final initialSettings = family_models.FamilySettings.defaultSettings()
            .copyWith(shareClassifications: true);
        final familyWithSettings = createMockFamily(settings: initialSettings);
        ...
        final shareSwitch =
            find.widgetWithText(SwitchListTile, 'Share Classifications');
        await tester.tap(shareSwitch); // Toggle to false
        await tester.pumpAndSettle();

        final expectedSettings =
            initialSettings.copyWith(shareClassifications: false);
        verify(mockFamilyService.updateFamily(argThat(
            predicate<family_models.Family>((f) =>
                f.settings.shareClassifications ==
                expectedSettings.shareClassifications)))).called(1);
        expect(find.text('Share classifications setting updated to false'),
            findsOneWidget);
      });

      testWidgets(
          'Toggle "Show Member Activity" calls updateFamily and shows SnackBar',
          (WidgetTester tester) async {
        final initialSettings = family_models.FamilySettings.defaultSettings()
            .copyWith(showMemberActivity: true);
        final familyWithSettings = createMockFamily(settings: initialSettings);
        ...
        final activitySwitch =
            find.widgetWithText(SwitchListTile, 'Show Member Activity');
        await tester.tap(activitySwitch); // Toggle to false
        await tester.pumpAndSettle();

        final expectedSettings =
            initialSettings.copyWith(showMemberActivity: false);
        verify(mockFamilyService.updateFamily(argThat(
            predicate<family_models.Family>((f) =>
                f.settings.showMemberActivity ==
                expectedSettings.showMemberActivity)))).called(1);
        expect(find.text('Show member activity setting updated to false'),
            findsOneWidget);
      });
    });
    ...
    group('Admin Actions', () {
      final adminUser =
          createMockUserProfile(id: 'adminUser', familyId: 'adminFam');
      final regularMember = createMockUserProfile(
          id: 'memberUser',
          displayName: 'Regular Member',
          familyId: 'adminFam');
      final familyForAdmin = createMockFamily(id: 'adminFam', members: [
        createMockFamilyMember(
            userId: adminUser.id, role: family_models.UserRole.admin),
        createMockFamilyMember(
            userId: regularMember.id, displayName: 'Regular Member'),
      ]);
      final pendingInvite =
          createMockInvitation(familyId: 'adminFam', id: 'inviteToCancel');

      setUp(() {
        when(mockStorageService.getCurrentUserProfile())
            .thenAnswer((_) async => adminUser);
        when(mockFamilyService.updateMemberRole(any, any, any))
            .thenAnswer((_) async {});
      ...
      testWidgets('Cancel Invitation calls service method',
          (WidgetTester tester) async {
        await tester.pumpWidget(createTestableWidget(
            familyForAdmin, FamilyManagementScreen(family: familyForAdmin)));
        ...
        await tester.tap(find
            ...
            .first); // Assuming one invitation, find its menu
