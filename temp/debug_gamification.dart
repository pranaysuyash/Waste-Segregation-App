...
void main() async {
  ...
  WasteAppLogger.info('🎮 DEBUG: Starting gamification debug...',
      context: {'debug_type': 'gamification', 'mode': 'temp_script'});
  ...
  try {
    ...
    WasteAppLogger.wasteEvent(
        'test_classification_created', testClassification.category,
        context: {
    ...
    WasteAppLogger.gamificationEvent('processing_classification',
        context: {'classification_id': testClassification.itemName});
    ...
    WasteAppLogger.gamificationEvent('classification_processed',
    ...
    if (pointsEarned > 0) {
      WasteAppLogger.gamificationEvent('points_verification_success',
          pointsEarned: pointsEarned);
    } else {
      WasteAppLogger.severe('Points verification failed',
          context: {'expected_points': '>0', 'actual_points': pointsEarned});
    }

    // Test cloud storage service gamification processing
    WasteAppLogger.info('Testing CloudStorageService gamification...',
        context: {'test_type': 'cloud_storage_gamification'});
    ...
    final cloudPointsEarned =
        profileAfterCloud.points.total - profileAfter.points.total;
    WasteAppLogger.gamificationEvent('cloud_points_earned',
        pointsEarned: cloudPointsEarned,
        context: {'service_type': 'cloud_storage'});
  } catch (e, stackTrace) {
    WasteAppLogger.severe('Gamification debug error',
        error: e,
        stackTrace: stackTrace,
        context: {'debug_type': 'gamification_temp_script'});
  }

  WasteAppLogger.info('🎮 DEBUG: Debug complete',
      context: {'debug_session': 'gamification_completed'});
  exit(0);
}
