...
void main(List<String> args) async {
  ...
  final priorityArg =
      args.where((arg) => arg.startsWith('--priority=')).firstOrNull;
...
class DebugPrintConverter {
  ...
  Future<void> convertAll({bool dryRun = false}) async {
    ...
    final result = await Process.run(
        'grep', ['-R', '-l', 'debugPrint(', 'lib/', '--include=*.dart']);
    ...
    print(
        '🎉 Conversion complete: $totalConverted total conversions across ${files.length} files');
  }
  ...
  Future<int> convertFile(String filePath, {bool dryRun = false}) async {
    ...
    final hasWasteAppLoggerImport = content
            .contains("import '../utils/waste_app_logger.dart'") ||
    ...
    for (final match in matches.reversed) {
      ...
      final replacement =
          _generateWasteAppLoggerCall(message, filePath, additionalArgs);
  ...
  String _generateWasteAppLoggerCall(
      String message, String filePath, String? additionalArgs) {
  ...
  String _generateSevereCall(String message, String fileName,
      String serviceType, String? additionalArgs) {
  ...
  String _generateWarningCall(String message, String fileName,
      String serviceType, String? additionalArgs) {
    final cleanMessage = _cleanMessage(message);
  ...
  String _generateCacheEventCall(String message, String fileName,
      String serviceType, String? additionalArgs) {
    final cleanMessage = _cleanMessage(message);
    final eventType = message.contains('hit')
        ? 'cache_hit'
        : message.contains('miss')
            ? 'cache_miss'
            : 'cache_operation';
  ...
  String _generatePerformanceCall(String message, String fileName,
      String serviceType, String? additionalArgs) {
    final cleanMessage = _cleanMessage(message);
  ...
  String _generateAIEventCall(String message, String fileName,
      String serviceType, String? additionalArgs) {
    final cleanMessage = _cleanMessage(message);
  ...
  String _generateUserActionCall(String message, String fileName,
      String serviceType, String? additionalArgs) {
    final cleanMessage = _cleanMessage(message);
  ...
  String _generateInfoCall(String message, String fileName, String serviceType,
  ...
  String _handleComplexDebugPrints(String content, String filePath) {
    ...
    content = content.replaceAllMapped(complexRegex, (match) {
      ...
      if ((messageExpression.startsWith("'") &&
              messageExpression.endsWith("'")) ||
          (messageExpression.startsWith('"') &&
              messageExpression.endsWith('"'))) {
        final message =
            messageExpression.substring(1, messageExpression.length - 1);
        final cleanMessage = _cleanMessage(message);
        return "WasteAppLogger.info('$cleanMessage', null, null, {'service': '$serviceType', 'file': '$fileName'});";
      }
  ...
  String _getServiceType(String filePath) {
    if (filePath.contains('services/')) {
      return filePath
          .split('/')
          .last
          .replaceAll('_service.dart', '')
          .replaceAll('.dart', '');
    } else if (filePath.contains('providers/')) {
      return filePath
          .split('/')
          .last
          .replaceAll('_provider.dart', '')
          .replaceAll('.dart', '');
