...
void main(List<String> args) async {
  ...
  if (!await projectDir.exists()) {
    WasteAppLogger.severe(
        'Error: Project directory not found. Run this script from the project root.');
    exit(1);
  }
  ...
  var updatedFiles = 0;
  final unusedImportRegex =
      RegExp(r"warning • Unused import: '.*' • (.*) • unused_import");
  ...
  WasteAppLogger.info(
      'Found \\${filesWithUnusedImports.length} files with unused imports.');
  ...
  for (final filePath in filesWithUnusedImports) {
    ...
    for (final importLine in importLines) {
      final importPath = importLine.split(' ')
        ..remove('import')
        ..removeWhere((e) => e.isEmpty);
      final importPackage =
          importPath.first.replaceAll("'", '').replaceAll('"', '');
    ...
    if (originalContent != content) {
      if (dryRun) {
        WasteAppLogger.info(
            '[DRY RUN] Would remove unused imports from \\${file.path}');
      } else {
