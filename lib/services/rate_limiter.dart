...
class RateLimiter {
  ...
  Future<void> acquire() async {
    ...
    if (enableLogging) {
      WasteAppLogger.info('Rate limit reached, queuing request', null, null, {
        'current_requests': _requestTimes.length,
        'max_requests': maxRequests,
        'window_duration_ms': windowDuration.inMilliseconds,
        'queue_length': _pendingRequests.length,
      });
    }
  ...
  bool _canMakeRequest(DateTime now) {
    ...
    if (burstLimit != null) {
      final recentRequests = _countRecentRequests(now, const Duration(seconds: 1));
  ...
  void _recordRequest(DateTime timestamp) {
    ...
    if (enableLogging) {
      WasteAppLogger.fine('Request recorded', null, null, {
  ...
  void _processPendingRequests() {
    ...
    while (_pendingRequests.isNotEmpty && _canMakeRequest(now)) {
      ...
      if (enableLogging) {
        WasteAppLogger.fine('Processed queued request', null, null, {
  ...
  Map<String, dynamic> getStatistics() {
    ...
    return {
      ...
      'recent_requests_1s': _countRecentRequests(now, const Duration(seconds: 1)),
      'recent_requests_10s': _countRecentRequests(now, const Duration(seconds: 10)),
  ...
  void reset() {
    ...
    if (enableLogging) {
      WasteAppLogger.info('Rate limiter reset', null, null, {
        'max_requests': maxRequests,
        'window_duration_ms': windowDuration.inMilliseconds,
      });
    }
...
class CostAwareRateLimiter extends RateLimiter {
  ...
  double get currentCost => _requestTimes.length * costPerRequest * costMultiplier;
  ...
  bool _canMakeRequest(DateTime now) {
    ...
    if (currentCost + requestCost > maxCostPerWindow) {
      if (enableLogging) {
        WasteAppLogger.warning('Cost limit reached', null, null, {
  ...
  void updateCostMultiplier(double newMultiplier) {
    ...
    if (enableLogging) {
      WasteAppLogger.info('Cost multiplier updated', null, null, {
  ...
  Map<String, dynamic> getStatistics() {
    ...
    return {
      ...
      'cost_utilization_percent': (currentCost / maxCostPerWindow * 100).round(),
    };
  }
}